...
``dataset_schema`` standard-root contract) — and consumes one or more **standard dataset
roots** (each with a ``manifest.json`` + per-clip target/prior mels + optional conditioning
tracks) emitted by ``DataSynthesizer`` (synthetic) and ``Labeler`` (recorded ground truth).
